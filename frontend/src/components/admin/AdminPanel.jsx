...
const AdminPanel = () => {
  ...
  const [programForm, setProgramForm] = useState({ title: '', category: '', description: '', image: '', price_usd: 0, price_inr: 0, price_eur: 0, price_gbp: 0, price_aed: 0, visible: true, order: 0, program_type: 'online', session_mode: 'online', enable_online: true, enable_offline: true, enable_in_person: false, offer_price_aed: 0, offer_price_usd: 0, offer_price_inr: 0, offer_text: '', is_upcoming: false, is_flagship: false, is_group_program: false, replicate_to_flagship: false, start_date: '', end_date: '', deadline_date: '', enrollment_open: true, enrollment_status: 'open', duration_tiers: [], whatsapp_group_link: '', zoom_link: '', custom_link: '', custom_link_label: '', show_whatsapp_link: true, show_zoom_link: true, show_custom_link: true, content_sections: [], timing: '', time_zone: '', show_duration_on_page: false, show_start_date_on_page: false, show_timing_on_page: false, show_duration_on_card: true, exclusive_offer_enabled: false, exclusive_offer_text: 'Limited Time Offer', closure_text: 'Registration Closed', show_pricing_on_card: true, show_tiers_on_card: true });
                ...
                <div data-testid="program-form" className="bg-white rounded-lg p-6 mb-6 shadow-sm border">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <div><Label>Timezone</Label><Input value={programForm.time_zone||''} onChange={e => setProgramForm({...programForm, time_zone: e.target.value})} placeholder="e.g., GST (Dubai) / IST" /></div>
                      ...
                      <p className="text-xs font-semibold text-gray-600 uppercase tracking-wider mb-2">Show on Program Page Header</p>
                      <div className="flex flex-wrap gap-4">
                        <label className="flex items-center gap-2 text-sm cursor-pointer"><input type="checkbox" checked={programForm.show_duration_on_page} onChange={e => setProgramForm({...programForm, show_duration_on_page: e.target.checked})} className="rounded" /><span>Duration</span></label>
                        <label className="flex items-center gap-2 text-sm cursor-pointer"><input type="checkbox" checked={programForm.show_start_date_on_page} onChange={e => setProgramForm({...programForm, show_start_date_on_page: e.target.checked})} className="rounded" /><span>Start Date</span></label>
                        <label className="flex items-center gap-2 text-sm cursor-pointer"><input type="checkbox" checked={programForm.show_timing_on_page} onChange={e => setProgramForm({...programForm, show_timing_on_page: e.target.checked})} className="rounded" /><span>Timing & Timezone</span></label>
                      </div>
                      <p className="text-xs font-semibold text-gray-600 uppercase tracking-wider mt-3 mb-2">Show on Upcoming Card</p>
                      <div className="flex flex-wrap gap-4">
                        <label className="flex items-center gap-2 text-sm cursor-pointer"><input type="checkbox" checked={programForm.show_duration_on_card !== false} onChange={e => setProgramForm({...programForm, show_duration_on_card: e.target.checked})} className="rounded" /><span>Duration Badge</span></label>
                      </div>
                    </div>
                    ...
                    {/* Exclusive Offer */}
                    <div className="col-span-2 border border-red-200 rounded-lg p-3 bg-red-50/50">
                      <div className="flex items-center gap-2 mb-2"><Switch checked={programForm.exclusive_offer_enabled} onCheckedChange={v => setProgramForm({...programForm, exclusive_offer_enabled: v})} /><Label className="text-red-700 font-semibold text-xs">Exclusive Offer Badge (shown on card)</Label></div>
                      {programForm.exclusive_offer_enabled && (
                        <Input value={programForm.exclusive_offer_text} onChange={e => setProgramForm({...programForm, exclusive_offer_text: e.target.value})} placeholder="e.g., Limited Time Offer, Limited Seats, Early Bird Special" className="bg-white" />
                      )}
                    </div>

                    {/* Toggles */}
                    <div className="flex items-center gap-2"><Switch checked={programForm.is_flagship} onCheckedChange={v => setProgramForm({...programForm, is_flagship: v})} /><Label>Flagship Program</Label></div>
                    ...
                    <div className="flex items-center gap-2"><Switch checked={programForm.is_group_program || false} onCheckedChange={v => setProgramForm({...programForm, is_group_program: v})} /><Label>Group Program</Label></div>
                    <div>
                      <Label>Enrollment Status</Label>
                      <select value={programForm.enrollment_status || 'open'} onChange={e => setProgramForm({...programForm, enrollment_status: e.target.value, enrollment_open: e.target.value === 'open'})}
                    ...
                    {(programForm.enrollment_status === 'closed' || (!programForm.enrollment_status && programForm.enrollment_open === false)) && (
                      <div className="col-span-2 border border-orange-200 rounded-lg p-3 bg-orange-50/50">
                        <Label className="text-orange-700 font-semibold text-xs mb-2 block">Closure Badge Text (shown on card)</Label>
                        <select value={programForm.closure_text} onChange={e => setProgramForm({...programForm, closure_text: e.target.value})} className="w-full border rounded-md px-3 py-2 text-sm bg-white">
